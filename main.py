...
def go_to_page(page_name):
    ...
    # 페이지 이동 시 복습 관련 세션 상태 초기화
    st.session_state.current_review_index = 0
    st.session_state.today_review_items = []
    st.session_state.selected_review_notes = [] # 선택 복습 목록도 초기화
...
# --- 복습 모듈 (오늘의 복습 및 선택 복습에서 공용으로 사용) ---
# 이 함수는 복습 로직을 캡슐화하여 'return' 오류를 방지합니다.
def review_module(review_items, is_manual_review_mode=False):
    today = datetime.now().date()
    ...
    # 복습 모드 선택 (기본은 추천 모드)
    # key에 is_manual_review_mode를 추가하여 수동/자동 복습 시 위젯 key 충돌 방지
    selected_review_mode = st.radio(
        "복습 모드 선택:",
        ["플래시카드", "주관식"],
        index=0 if current_note['initial_review_mode'] == '플래시카드' else 1,
        key=f"mode_select_{current_note['id']}_{st.session_state.current_review_index}_{is_manual_review_mode}" 
    )
    ...
    if selected_review_mode == "플래시카드":
        ...
        unique_key_show_back = f"show_back_{current_note['id']}_{st.session_state.current_review_index}_{is_manual_review_mode}"
        if unique_key_show_back not in st.session_state:
            st.session_state[unique_key_show_back] = False

        if st.button("정답 확인", key=f"show_back_btn_{current_note['id']}_{st.session_state.current_review_index}_{is_manual_review_mode}"):
    ...
    # 주관식 모드 (Q&A 형식 노트에 적합)
        ...
        unique_key_user_answer = f"user_answer_{current_note['id']}_{st.session_state.current_review_index}_{is_manual_review_mode}"
        unique_key_answer_checked = f"answer_checked_{current_note['id']}_{st.session_state.current_review_index}_{is_manual_review_mode}"
        ...
        user_answer = st.text_area("답변을 입력하세요.", value=st.session_state[unique_key_user_answer], key=f"user_answer_text_{current_note['id']}_{st.session_state.current_review_index}_{is_manual_review_mode}")
        
        if st.button("정답 확인", key=f"check_answer_btn_{current_note['id']}_{st.session_state.current_review_index}_{is_manual_review_mode}"):
    ...
    with col1:
        if st.button("😊 쉬웠음", key=f"diff_easy_{current_note['id']}_{st.session_state.current_review_index}_{is_manual_review_mode}"):
            selected_difficulty = "쉬웠음"
            difficulty_chosen = True
    with col2:
        if st.button("🙂 보통", key=f"diff_normal_{current_note['id']}_{st.session_state.current_review_index}_{is_manual_review_mode}"):
            selected_difficulty = "보통"
            difficulty_chosen = True
    with col3:
        if st.button("🙁 어려웠음", key=f"diff_hard_{current_note['id']}_{st.session_state.current_review_index}_{is_manual_review_mode}"):
            selected_difficulty = "어려웠음"
            difficulty_chosen = True
    with col4:
        if st.button("😩 전혀 기억 안 남", key=f"diff_forgot_{current_note['id']}_{st.session_state.current_review_index}_{is_manual_review_mode}"):
    ...
    if difficulty_chosen and selected_difficulty: # 선택된 난이도가 있을 때만 진행
        ...
        for key_prefix in [f"show_back_{current_note['id']}", f"user_answer_{current_note['id']}", f"answer_checked_{current_note['id']}"]:
            ...
            # is_manual_review_mode 값도 key의 일부이므로 함께 고려하여 삭제
            key_to_delete = f"{key_prefix}_{st.session_state.current_review_index}_{is_manual_review_mode}"
...
# --- 오늘의 복습 페이지 ---
    ...
    # 복습 모듈 호출 (이곳에서 모든 복습 로직 처리)
    review_module(st.session_state.today_review_items, is_manual_review_mode=False)
...
# --- 선택 복습 페이지 ---
    ...
    if not st.session_state.notes:
        ...
        # 복습이 시작되었거나 진행 중인 경우, 또는 모든 복습이 완료된 경우
            ...
            review_module(st.session_state.selected_review_notes, is_manual_review_mode=True)
